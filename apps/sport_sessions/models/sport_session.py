...
class SportSession(models.Model):
    ...
    # Liste extensible des sports supportés
    SPORT_CHOICES = [
        ('football', 'Football'),
    ...
    sport = models.CharField(max_length=50, choices=SPORT_CHOICES, help_text="Type de sport.")
    description = models.TextField(blank=True, null=True, help_text="Détails ou règles de la session.")
    location = models.CharField(max_length=255, help_text="Lieu où se déroulera la session.")
    date = models.DateField(help_text="Date de la session.")
    start_time = models.TimeField(help_text="Heure de début de la session.")

    # Paramètres d'inscription
    is_public = models.BooleanField(default=True, help_text="Session ouverte à tous les utilisateurs.")
    team_mode = models.BooleanField(default=False, help_text="Active les équipes pour les sports à adversité.")
    max_players = models.PositiveIntegerField(default=10, validators=[MinValueValidator(1)], help_text="Nombre total maximum de participants.")
    min_players_per_team = models.PositiveIntegerField(null=True, blank=True, validators=[MinValueValidator(1)], help_text="Minimum de joueurs par équipe (optionnel, si team_mode).")
    max_players_per_team = models.PositiveIntegerField(null=True, blank=True, validators=[MinValueValidator(1)], help_text="Maximum de joueurs par équipe (optionnel, si team_mode).")
    ...
    def __str__(self):
        return f"{self.title} - {self.get_sport_display()} ({self.date})"
